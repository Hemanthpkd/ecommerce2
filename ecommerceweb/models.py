...
class Products(models.Model):
    ...
    # using custom method to substract product's actual price and offer price
    
    @property
    def offer_price(self):
        # here self means a single product
        offs=Offers.objects.filter(product=self)
        # here the reason why we are giving filter instead of get is because there could be a chance
        #  for the absense of offers
        if offs:
            off=offs[0]
